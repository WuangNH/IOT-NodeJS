...
function updateSensorData(data) {
  ...
  const imgElement2 = document.getElementById("light-icoin1");
  ...
  if (data.length > 0) {
    ...
    dustDisplay.textContent = latestData.db;
     ...
     if (latestData.db > 500) {
      flashRedWhite(valueDisplay4);
      flashRed(imgElement2);
      turnNNRelay1()
    }
    else {
...
function flashRed(imgElement) {
  let isRed = false;
  const intervalId = setInterval(function () {
    if (isRed) {
      imgElement.style.filter = "hue-rotate(0deg)"; // Đổi lại màu gốc
    } else {
      imgElement.style.filter = "hue-rotate(90deg)"; // Đổi màu đỏ
    }
    isRed = !isRed;
  }, 50); // Thay đổi màu sau mỗi 500ms

  // Dừng nhấp nháy sau một khoảng thời gian (ví dụ: sau 5 giây)
  setTimeout(function () {
    clearInterval(intervalId); // Dừng interval
    imgElement.style.filter = "hue-rotate(0deg)"; // Đảm bảo màu cuối cùng là màu gốc
  }, 2000); // Dừng sau 5 giây
}


function flashRedWhite(element) {
  let isRed = true;
  ...
  const intervalId = setInterval(function () {
    ...
    isRed = !isRed;
  }, 50); // Đổi màu sau mỗi 500ms

  // Dừng nhấp nháy sau một khoảng thời gian (ví dụ: sau 5 giây)
  setTimeout(function () {
    clearInterval(intervalId); // Dừng interval
    element.style.backgroundColor = "white"; // Đảm bảo nền cuối cùng là trắng
  }, 2000); // Dừng sau 5 giây
}
